...
def year_wise_medal_tally(df,country):
    df_2 = df.dropna(subset=['Medal'])
    df_2.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Year', 'City', 'Sport', 'Event', 'Medal'], inplace=True)

    new_df = df_2[df_2['region'] == country]
...
def weight_v_height(df,sport):
    at_df = df.drop_duplicates(subset = ['Name','region'])
    at_df['Medal'].fillna('No Medal',inplace = True)

    if sport != 'Overall':
        temp_df = at_df[at_df['Sport'] == sport]
        return temp_df
    else:
        return at_df
...
def men_v_women(df):
    at_df = df.drop_duplicates(subset=['Name', 'region'])
    men = at_df[at_df['Sex'] == 'M'].groupby('Year').count()['Name'].reset_index()
    women = at_df[at_df['Sex'] == 'F'].groupby('Year').count()['Name'].reset_index()
