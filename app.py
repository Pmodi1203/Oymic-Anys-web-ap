...
df = pd.read_csv('/Users/pavanmodi/Desktop/Projects /Olympic Data Analysis with ML/athlete_events.csv')
region_df = pd.read_csv('/Users/pavanmodi/Desktop/Projects /Olympic Data Analysis with ML/noc_regions.csv')

df = preprocessor.preprocess(df,region_df)


st.sidebar.title("Olympic Analysis")
st.sidebar.image('https://static.toiimg.com/thumb/msid-84847838,imgsize-103235,width-400,resizemode-4/84847838.jpg')
...
if user_menu == 'Medal Tally':
    ...
    if selected_year == 'Overall' and selected_country != 'Overall':
        st.title("Overall Medal Tally for " + selected_country + " Country")

    if selected_year != 'Overall' and selected_country != 'Overall':
        st.title(selected_country + "'s Medal Tally for Year " + str(selected_year))
...
if user_menu == 'Overall Analysis':
    ...
    fig = px.line(athlets_over_time, x="Editions", y="Name")
    st.title("Athletes over the years")
    st.plotly_chart(fig)

    st.title("Number of Events over a time(every Sports)")
...
if user_menu =='Athlete wise Analysis':
    ...
    for sport in famous_sport:
        ...
        pav = temp_df[temp_df['Medal'] == 'Gold']['Age'].dropna()
        x.append(pav)
        name.append(sport)

    fig = ff.create_distplot(x, name, show_hist=False, show_rug=False)
    fig.update_layout(autosize=False, width=1000, height=600)
    st.title(' Distribution of Age for sports(Gold Medalist)')
    st.plotly_chart(fig)
